...
- `[DECIDED]` Auto‑generated `.d.ts` stubs from the KConfigXT schema (`config/main.xml`) and any QML singletons we expose, so the TS layer types its QML‑facing APIs. Generator script lives in `scripts/gen-types.ts`, runs as a build step before `bun build`.
